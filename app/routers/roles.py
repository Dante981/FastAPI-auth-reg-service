...
@router.delete("/{role_code}", dependencies=[Depends(require_permission("roles:update"))])
async def delete_role(role_code:str, payload: RoleUpdate, db: AsyncSession = Depends(get_db)) -> None:
    return await update_role_by_code(payload=payload, role_code=role_code, db=db)
