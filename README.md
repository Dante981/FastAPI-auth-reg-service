...
### Роли и доступы
В проекте роль лучше проверять по стабильному `code`, а не по `id`.

`id` — это внутренний технический идентификатор, который может меняться при миграциях, пересоздании данных или переносе базы.  
`code` — это постоянный логический идентификатор роли, например `admin`, `user` или `moderator`, и именно его удобнее использовать в логике доступа.
